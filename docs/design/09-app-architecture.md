...
**Судьба TS-прототипа `compute-core/` (миграционный план).** Существующий TypeScript
`compute-core/` в корне репо — ранний прототип (2026-05-29), полезный как референс
пайплайна и источник golden-фикстур, но **НЕ спецификация ядра**: выход у него точечный
(не NormValue-диапазон), формулы гл.07 не реализованы, план пола отдаётся SVG-строкой
(вопреки D2). Источник истины контракта — **гл.05 §8 + гл.07**; Rust-порт пишется по
ним в `crates/compute-core` (TS-типы генерятся ИЗ Rust, D3/D4). При старте слайса 0:
`cargo new crates/compute-core` (+ `rust-toolchain.toml` с пином stable);
TS-прототип замораживается на месте как референс и удаляется (или переезжает в
`reference/`) после того, как Rust-ядро воспроизведёт его golden-фикстуры.
