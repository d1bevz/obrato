...
`docs/design/08-mockups/index.html` — self-contained визуальные макеты ключевых экранов
(S1–S6) в индустриально-приборной эстетике: высокий контраст под солнце, крупные таргеты
под перчатку, единый язык диапазонов/confidence (§4), реальный `<canvas>` для LayoutCanvas
(naive grid + tap-to-cut). Открывается в браузере без сборки; продакшен — React +
императивный Canvas (гл. 09 ①).
