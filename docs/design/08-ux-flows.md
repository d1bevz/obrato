...
| *(не галочка работ)* **Мокрая зона** | `Room.wet: bool` | включает ветку **гидроизоляция + плитка**; драйвит этапы waterproofing/tiling. При `wet=true` раскрывается поле `wet_zone_height_m` — высота захода гидро на стены (дефолт из шаблона, гл. 05 §2): площадь гидры = пол + периметр × заход (гл. 07 §1.3) |
...
**Norm-входы вне ввода прораба (явная граница пилота).** Остальные входы формул гл. 07
(зуб гладилки, ширина шва, формат плитки A×B×C, впитываемость основания, толщина слоя,
ровность) в S3 **НЕ вводятся** — они берутся как seed-дефолты `NormAssumptions`
(гл. 05 §3) с широким диапазоном и `confidence=unvalidated`, и уточняются калибровкой
по факту (Risk #1), а не полями формы. Это сознательное решение П3 (минимум ввода),
а не пропуск. Единственное исключение — `wet_zone_height_m` (заход гидро): без него
площадь гидроизоляции невыразима в принципе (гл. 07 §1.3, §3.4), поэтому это поле
комнаты при `wet=true`, а не зашитое допущение.
...
Бейджи дельты называют **конкретную причину**, а не абстрактную «правку»
(заимствовано из параллельной проработки — ветка `archimedes/ux-flows`):

- **Шаблон** (серый) — `WorkSelection.source = template`: дефолт, можно не читать детали.
- **Изменено** (оранжевый) — `source = user`: UI называет, что именно тронуто
  («розетки и свет изменены», «пол выключен»).
- **Проверить** (жёлтый) — расчёт строки зависит от входа Risk #1 (`NormAssumptions`,
  гл. 07: ровность основания, шов плитки, впитываемость, высота захода гидроизоляции).
  Это **производное** UI-состояние — вид на `NormValue.confidence` и поля ассампшнов;
  отдельного поля статуса в данных **нет** и не заводится.
...
- **Элементы:** выбор `RoomType` (предзаполняет всё); три больших поля **Д×Ш×В**
  (`measurement_source`); тумблер **`wet`** (при включении — поле `wet_zone_height_m`,
  заход гидро на стены, дефолт из шаблона, §3.2); **4 галочки работ** с раскрытием
  минимального параметра (§3.2); проёмы (`Opening{kind,width,height,count}`); тултип
  допущений → NormRangeView. Валидация физики (>0, проём ≤ стены, гл. 09 ①).
...
### 10.1 Кто он
Спокойный компетентный **напарник-прораб** — **дружелюбное мультяшное** лицо (намеренно
**НЕ** маска Гая Фокса): круглое лицо, **большие круглые глаза с бликами**, нос-бульбочка,
**жёлтая каска** (как на реальной стройке) и фирменные **усы-хэндлбар** (густое тело,
тонкие закрученные **вверх** кончики — подпись персонажа). Без отдельных бровей (чтобы не
было «четырёх глаз»/маски). Бренд-акцент интерфейса — зелёный; каска жёлтая (реализм), и
вместе они не конфликтуют. Эмоцию несут глаза/наклон головы и реквизит. Регистр копирайта:
на «ты», по-мастеровому, без сюсюканья. Две формы:
...
`NormValue.confidence` + калибр-флаг (§4); «успех» = freeze/PurchaseSnapshot (S5, гл. 05 §5).
...
3. **Когда усы есть — кончики вверх** (пышный хэндлбар, инвариант подписи). Единственное
   исключение — **офлайн убирает усы целиком** (§10.5/06); состояния «усы вниз» не существует.
   **Каска жёлтая** (реализм стройки), бренд-акцент UI — зелёный; цвета не конфликтуют.
...
6. **Motion-бюджеты** (заимствовано из `archimedes/ux-flows`): micro-реакция 120–320 мс,
   смена состояния 320–650 мс, idle-цикл 2.8–4 с. Анимация **не меняет габариты карточки**
   (никакого layout shift) и **не закрывает** CTA, поля ввода, цену/диапазон или canvas.
   Без звука; haptic по умолчанию выключен.

### 10.4 Триггеры — что вообще будит Amigo

Сначала **инвентарь триггеров**, эмоции — вторым проходом поверх него, иначе позы
дублируют друг друга (метод заимствован из параллельной проработки — ветка
`archimedes/ux-flows`). Только эти события имеют право разбудить маскота:

| Группа | Триггер | Где | Реакция (сцена §10.5) |
|---|---|---|---|
| Возврат | открыт список с активным объектом / next-step на хабе | S1, S2 | 01 — подсказать следующий шаг |
| Ввод | комната из шаблона; размер стал валидным; дельта работ/wet | S3 | 02 — подтвердить ввод, объяснить шаблон |
| Ввод | commit геометрии (debounce на blur, Q#1); физически невозможная геометрия | S3 | «считаю…» / блокирующая проверка без паники |
| Чертёж | открыт canvas с хинтом; добавлен cut/suppress; overrides stale | S4 | 03 — хинт tap-to-cut, подтверждение `CutOverride` |
| Закупка | wide-range строка; раскрыто «почему широко» | S5 | 04/05 — объяснить диапазон без fake precision |
| Freeze | «взять в магазин» нажато / snapshot создан | S5 | 04 — freeze-«считаю» → успех |
| Факт | факт записан; выбран rough («для истории, нормы не уточняет») | S6 | 05 — связь факта с нормами, честно |
| Офлайн/sync | локальное сохранение без сети; sync завершён; retry | глобально | 06 — спокойная автономность, без красной паники |

**Анти-триггеры** (Amigo молчит): обычный скролл, каждый tap по списку, каждая цифра
numeric-ввода, просмотр цены, повторное появление уже прочитанной подсказки.
Attention-анимация одной подсказки — не чаще **одного раза за сессию экрана**; пока
прораб активно вводит данные, idle приглушается (не спорить с руками).

### 10.5 Эмоции по сценариям (зафиксировано)
...
### 10.6 Дизайн-токены (зафиксировано в макетах)

Единая палитра макетов S1–S6 и маскота, сведена в текст главы как контракт для
дизайн-системы (практика заимствована из `archimedes/ux-flows`; значения — наши,
канонический источник — `:root` в `08-mockups/index.html`):

| Токен | Значение | Использование |
|---|---|---|
| `--paper` | `#f3efe6` | фон приложения («бумага») |
| `--surface` | `#fffdf8` | карточки, строки, формы |
| `--ink` / `--ink-2` / `--ink-3` | `#1b1813` / `#6c6457` / `#9a9182` | текст: основной / вторичный / подписи |
| `--green` / `--green-d` | `#1f9d57` / `#14723d` | бренд / CTA / exact |
| `--helmet` / `--helmet-d` | `#f7b500` / `#d49400` | каска Amigo — только маскот, **не** UI-сигнал |
| `--cyan` | `#1c7e8c` | blueprint/чертёж, wet-зоны |
| `--hi` (+ `-bg`) | `#2f7d4f` (`#e2f0e6`) | confidence: exact / high (§4) |
| `--mid` (+ `-bg`) | `#d07d18` (`#fbeed6`) | confidence: estimated / medium (§4) |
| `--lo` (+ `-bg`) | `#c0492f` (`#f7e0d9`) | confidence: wide / low / unvalidated (§4) |
| шрифты | Saira Semi Condensed / Spline Sans Mono | UI / моноширинные цифры («прибор») |
...
- `docs/design/08-mascot/emotions.html` — **эмоции по сценариям** (§10.5): 6 сцен с реквизитом
