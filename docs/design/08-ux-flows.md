...
- **Персонаж только для состояния.** Helper появляется на empty/success/offline,
  compute и next-step состояниях. Он не заменяет данные, не закрывает список комнат
  и не становится главным героем рабочего экрана. Внешность - простой строительный
  helper с лёгкими закрученными усами, без карикатурности.
