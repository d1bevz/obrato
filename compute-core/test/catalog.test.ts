...
test('каталог курируемого размера: 50..250 SKU', () => {
  assert.ok(SEED.skus.length >= 50 && SEED.skus.length <= 250, `SKU = ${SEED.skus.length}, ожидалось 50..250`);
});

test('multi-store: leroy-pt + ≥8 магазинов, у каждого есть SKU', () => {
  const keys = SEED.stores.map((s) => s.key);
  assert.equal(new Set(keys).size, keys.length, 'дубли Store.key');
  assert.ok(keys.includes('leroy-pt'), 'нет якорного магазина leroy-pt');
  assert.ok(SEED.stores.length >= 8, `магазинов ${SEED.stores.length}, ожидалось ≥8`);
  ...
  for (const st of SEED.stores) {
    assert.ok((perStore.get(st.key) ?? 0) > 0, `магазин ${st.key} без SKU`);
  }
});
...
test('проекция покрывает все 9 видов материала, валидными SKU движка', () => {
  ...
  for (const key of MATERIAL_KEYS) {
    ...
    assert.ok(storeKeys.has(sku.store), `проекция ${key}: неизвестный store ${sku.store}`);
  }
